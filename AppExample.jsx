  ...
  return (
    <motion.div whileHover={{ y: -2 }}>
      ...
      >
        {children}
        <motion.span
          className="absolute bottom-0 left-0 w-full h-0.5 bg-indigo-400 origin-left scale-x-0 hover:scale-x-100 transition-transform"
          whileHover={{ scaleX: 1 }}
        />
      </Link>
    </motion.div>
...
function NavLink({ to, children }) { 
  return (
    <Link 
      to={to}
      className="text-white hover:text-amber-300 transition-colors px-3 py-2 rounded"
    >
      {children}
    </Link>
  )
...
function Footer() {
  ...
  React.useEffect(() => {
    // Use localStorage to persist visit count across sessions
  ...
  return (
    <footer className="bg-gray-800 text-white p-4 text-center">
      <div className="container mx-auto">
        <p>© 2024 AutoFinder. Все права защищены.</p>
        <p>Контактная информация: kpetcheko1tech.it@gmail.com | +7 (988) 115-504</p>
